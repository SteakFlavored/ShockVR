...
typedef struct {
    ...
    uint32_t offset;            // byte offset to chunk start
} MovieChunk;
...
#define MOVIE_TEXTITEM_MAKETAG(c1,c2,c3,c4) ((((uint32_t)c4)<<24)|(((uint32_t)c3)<<16)|(((uint32_t)c2)<<8)|(c1))
#define MOVIE_TEXTITEM_TAG(pmti,index) ((pmti+(index))->tag)
#define MOVIE_TEXTITEM_PTR(pmti,index) ((int8_t *)(pmti) + (pmti+(index))->offset)
...
void MovieInstallStdTextCallback(Movie *pmovie, uint32_t lang, Id fontId,

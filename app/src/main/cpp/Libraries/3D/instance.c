...
// externs
extern void angles_2_matrix(g3s_angvec *angles, g3s_matrix *view_matrix, int32_t rotation_order);
...
bool g3_start_object_angles_zy(g3s_vector *p,fixang ty,fixang tz,int32_t rotation_order);
bool start_obj_common(g3s_vector *p,g3s_angvec *o,int32_t rotation_order);
...
int8_t context_stack[MAX_INSTANCE_DEPTH * CONTEXT_SIZE];
int8_t *cstack_ptr = context_stack;

int32_t    cstack_depth;
...
bool g3_start_object_angles_v(g3s_vector *p,g3s_angvec *o,int32_t rotation_order)
 {
     if (save_context()) return 0;
...
// dest=c1*s1+c2*s2
static fix update_m(fix c1, fix s1, fix c2, fix s2)
{
    int64_t result = (int64_t)c1 * (int64_t)s1 +
            (int64_t)c2 * (int64_t)s2;
    return fix_from_int64(result);
}

// dest=c1*s1-c2*s2
static fix update_ms(fix c1, fix s1, fix c2, fix s2)
{
    int64_t result = (int64_t)c1 * (int64_t)s1 -
            (int64_t)c2 * (int64_t)s2;
    return fix_from_int64(result);
...
//rotate viewer vars
    result = update_ms(_view_position.gX, cos_y, _view_position.gZ, sin_y);
    temp1 = fix_from_int64(result);
    result = update_m(_view_position.gX, sin_y, _view_position.gZ, cos_y);
    _view_position.gX = temp1;
    _view_position.gZ = fix_from_int64(result);
...
//rotate viewer vars
    result = update_m(_view_position.gZ, sin_x, _view_position.gY, cos_x);
    temp1 = fix_from_int64(result);
    result = update_ms(_view_position.gZ, cos_x, _view_position.gY, sin_x);
...
//rotate viewer vars
    result = update_m(_view_position.gY, sin_z, _view_position.gX, cos_z);
    temp1 = fix_from_int64(result);
    result = update_ms(_view_position.gY, cos_z, _view_position.gX, sin_z);
    _view_position.gX = temp1;
    _view_position.gY = fix_from_int64(result);
